        ...
        if "epuck" in _normalize(alias):
            try:
                ...
                try:
                    ...
                    continue
    raise RuntimeError(
        "e-puck não encontrado. Verifique o path no Scene Hierarchy e adicione em EPUCK_PATH_HINTS."
    )
...
def find_wheel_joints(
    session: CoppeliaSimSession, epuck_handle: int
) -> Tuple[int, int]:
    sim = session.simulation
    ...
    for j in joints:
        ...
        if re.search(r"\b(left|lwheel|l_motor|l-?joint|leftmotor|motorleft)\b", name):
            ...
            r"\b(right|rwheel|r_motor|r-?joint|rightmotor|motorright)\b", name
        ):
            right = j
    ...
    if left is None or right is None:
        raise RuntimeError(
            "Juntas das rodas não identificadas. Renomeie (leftMotor/rightMotor) ou ajuste a regex."
        )
    return left, right
...
def main():
    with CoppeliaSimSession(host="127.0.0.1", port=23000, auto_connect=True) as sess:
        ...
        print(
            "rodas:",
            sess.simulation.getObjectAlias(left, 1),
            "|",
            sess.simulation.getObjectAlias(right, 1),
        )
